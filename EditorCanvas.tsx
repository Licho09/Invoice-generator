...
export default function EditorCanvas({
  ...
  textCursor: {x: number, y: number, visible: boolean};
  ...
  // PDF page dimensions in pixels (8.5 x 11 inches at 96 DPI)
  const PDF_PAGE_WIDTH = 816; // 8.5 * 96
  const PDF_PAGE_HEIGHT = 1056; // 11 * 96
  ...
  const updateTableCell = (tableId: number, row: number, col: number, value: string) => {
    setTables(prev => 
      prev.map(table => {
        if (table.id === tableId) {
  ...
  const onMouseMove = (e: MouseEvent) => {
    if (dragging && selectedId !== null) {
      ...
      startPos.current = { x: e.clientX, y: e.clientY };
    } else if (dragging && selectedTable !== null) {
      const dx = e.clientX - startPos.current.x;
      const dy = e.clientY - startPos.current.y;
      setTables((prev) =>
        prev.map((table) =>
    ...
    return (
      ...
      >
        ...
        <div 
          ...
          style={{
            gridTemplateColumns: `repeat(${table.cols}, ${table.cellWidth}px)`,
            gridTemplateRows: `repeat(${table.rows}, ${table.cellHeight}px)`,
          }}
        >
  ...
  return (
    <div className="flex-1 bg-gradient-to-br from-slate-50 to-slate-100 p-6 overflow-y-auto overflow-x-hidden">
      <div className="flex justify-center">
        <div
          className="relative bg-white rounded-xl shadow-xl border border-slate-200 p-8 overflow-hidden"
          style={{ 
        ...
        >
          ...
          <div className="absolute top-0 left-0 right-0 bg-gradient-to-r from-slate-600 to-slate-700 text-white p-4 rounded-t-xl">
            <h3 className="text-lg font-semibold">PDF Invoice Designer (8.5" x 11")</h3>
            <p className="text-slate-300 text-sm">Click to add text, drag to create shapes, insert tables</p>
          </div>
          ...
          <div className="pt-20 relative">
            ...
            {textCursor.visible && (
              <div
                className="absolute w-0.5 h-5 bg-black animate-pulse"
                style={{
                  left: textCursor.x,
                  top: textCursor.y,
                  zIndex: 20,
          ...
          {(shapes.length > 0 || tables.length > 0) && (
            <div className="absolute bottom-4 right-4 bg-white/90 backdrop-blur-sm border border-slate-200 rounded-lg px-3 py-2 shadow-lg">
              <span className="text-sm text-slate-600">
                {shapes.length} shape{shapes.length !== 1 ? 's' : ''}{tables.length > 0 && `, ${tables.length} table${tables.length !== 1 ? 's' : ''}`} on canvas
              </span>
            </div>
          )}
